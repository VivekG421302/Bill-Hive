...
- **Standalone pages:** `brands.html`, `suppliers.html`, `fulfillment.html` — separate HTML documents that share `styles.css` and `chrome.js`.
...
- **`installPWA()` is defined in:**
  ...
  - `chrome.js` — for `brands.html`, `suppliers.html`, `fulfillment.html`
- **Service worker registration** — inline `<script>` at the bottom of every HTML file registers `sw.js`

---

## Standalone pages (brands, suppliers, fulfillment)
...
- [ ] Is it a new page? Add section to `index.html`, link in sidebar (all 4 HTML files), add to `navigateTo()`.
