...
<head>
    ...
    <title>Fulfillment — Bill-Hive</title>
    <link rel="manifest" href="manifest.json">
    <meta name="theme-color" content="#228be6">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="apple-mobile-web-app-status-bar-style" content="default">
    <meta name="apple-mobile-web-app-title" content="Bill-Hive">
...
/* Logo is displayed at a 4:1 (width:height) aspect ratio */
.logo-container {
    width: 100%;
    max-width: 176px;
    height: 44px;
...
.line-item-fields .form-group input {
    padding: 8px 12px;
    font-size: 0.875rem;
}
...
/* Mobile swipe delete indicator */
.line-item-swipe-indicator {
    display: none;
}
...
@media (max-width: 640px) {
    ...
    .line-item.swiped .line-item-swipe-indicator {
        transform: translateX(0);
    }

    .line-item.swiped .line-item-content {
    ...
    .line-item-delete-mobile {
        display: flex;
        align-items: center;
        justify-content: center;
    }

    .line-item-delete-mobile svg {
        width: 20px;
        height: 20px;
    }
}
...
.pos-bill {
    max-width: 300px;
    margin: 0 auto;
    font-size: var(--pw-size, 11px);
    font-weight: var(--pw-base, 500);
}
...
.pos-table .pos-td-right {
    text-align: right;
}

.pos-table .pos-td-center {
    text-align: center;
}
...
.brand-card-logo-placeholder svg {
    width: 32px;
    height: 32px;
}
...
.data-view-logo-wrap img {
    width: 100%;
    height: 100%;
    object-fit: contain;
}

.data-view-logo-placeholder {
    display: flex;
    align-items: center;
    gap: 8px;
    color: var(--text-muted);
...
.data-view-logo-placeholder svg {
    width: 22px;
    height: 22px;
    flex-shrink: 0;
}

.data-view-logo-placeholder span {
    font-size: 0.8125rem;
    white-space: nowrap;
}
...
@media (max-width: 640px) {
    ...
    .form-group.optional-field.show {
        display: flex;
    }
...
/* Highlighted stat cards (Net Sales Today / Monthly) */
.stat-card-highlight {
...
.pwa-install-btn:hover {
    opacity: 0.9;
    background: linear-gradient(135deg, var(--accent-primary-hover) 0%, var(--accent-info) 100%) !important;
}
.pwa-install-btn svg {
    color: #fff !important;
}

/* Items table responsive: hide less-critical columns on small screens */
@media (max-width: 640px) {
...
/* Header brand icon (favicon) on the left of the header */
.header-brand-icon {
    width: 34px;
    height: 34px;
    border-radius: var(--radius-md);
...
/* Item name search — custom dropdown (replaces unreliable native datalist popup) */
.line-item-name-wrap {
    position: relative;
    flex: 1;
    min-width: 0;
...
.item-suggestion-row:hover,
.item-suggestion-row.active {
    background: var(--bg-hover);
}
...
/* Sales Return page — breathing room between the invoice picker, the
   returnable-items table and the refund summary block */
#return-items-container:not(:empty) {
    margin-top: 18px;
}
...
/* Extra vertical spacing between stacked form rows inside any modal or
   bill section — fixes the cramped look across popups app-wide */
.modal-body .form-grid,
.bill-section .form-grid {
    row-gap: 18px;
}
...
/* Stock adjust modal — "don't add to inventory" note */
.stock-adjust-hint {
    font-size: 0.75rem;
    color: var(--text-muted);
    margin-top: 6px;
...
/* Task 3: Header/Footer Edge Fading (Claude-style seamless scroll)
   The ::after on the header and ::before on the footer cast a short
   gradient shadow so content appears to slide cleanly under/over them. */

.app-header {
    overflow: visible; /* allow pseudo-element to bleed below */
}
...
.app-footer {
    overflow: visible; /* allow pseudo-element to bleed above */
}
...
<body class="light-mode">
    ...
    <aside id="sidebar" class="sidebar">
        <div class="sidebar-header">
            <div class="sidebar-brand">
                <svg class="sidebar-logo" viewBox="0 0 48 48" fill="none">
                    <path d="M24 2L4 13v22l20 11 20-11V13L24 2z" stroke="currentColor" stroke-width="2.5" fill="none"/>
                    <path d="M24 2v22M4 13l20 11 20-11" stroke="currentColor" stroke-width="2" fill="none"/>
                    <rect x="14" y="30" width="20" height="14" rx="2" stroke="currentColor" stroke-width="2" fill="none"/>
                    <line x1="18" y1="34" x2="30" y2="34" stroke="currentColor" stroke-width="1.5"/>
                    <line x1="18" y1="38" x2="26" y2="38" stroke="currentColor" stroke-width="1.5"/>
                    <path d="M28 38l2 2 4-4" stroke="currentColor" stroke-width="1.5" fill="none"/>
                    <line x1="14" y1="44" x2="34" y2="44" stroke="currentColor" stroke-width="1.5" stroke-dasharray="3 2"/>
                </svg>
                <span class="sidebar-title">Bill-Hive</span>
            </div>
        ...
        <nav class="sidebar-nav">
            <a href="index.html#create-bill" class="sidebar-link" data-page="create-bill">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="index.html#past-bills" class="sidebar-link" data-page="past-bills">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="index.html#items" class="sidebar-link" data-page="items">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="index.html#stock" class="sidebar-link" data-page="stock">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M22 12h-4l-3 9L9 3l-3 9H2"/>
                </svg>
                <span>Stock</span>
            </a>
            <a href="index.html#sales-return" class="sidebar-link" data-page="sales-return">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="index.html#sale-summary" class="sidebar-link" data-page="sale-summary">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="brands.html" class="sidebar-link" data-page="brands">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="suppliers.html" class="sidebar-link" data-page="suppliers">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="fulfillment.html" class="sidebar-link active" data-page="fulfillment">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                </svg>
                <span>Fulfillment</span>
            </a>
            <a href="catalogue.html" class="sidebar-link" data-page="catalogue">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="sidebar-divider"></div>
            <a href="index.html#your-data" class="sidebar-link" data-page="your-data">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <a href="index.html#settings" class="sidebar-link" data-page="settings">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="sidebar-divider"></div>
            <button class="sidebar-link theme-toggle-btn" onclick="toggleTheme()">
            ...
            <div class="sidebar-divider" id="pwa-install-divider" style="display:none;"></div>
            <button class="sidebar-link pwa-install-btn" id="pwa-install-btn" onclick="installPWA()" style="display:none;">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <header class="app-header">
        <div class="header-left">
            <img src="favicon.svg" class="header-brand-icon" alt="Bill-Hive">
        </div>
        <div class="header-center">
            <div class="logo-container" id="header-logo-container">
                <img id="header-logo" src="" alt="Company Logo" style="display:none;">
                <div class="logo-placeholder" id="header-logo-placeholder">
                    ...
                    <span class="logo-placeholder-text">Bill-Hive</span>
                </div>
            </div>
        </div>
        <div class="header-right">
            <button class="header-menu-btn" onclick="openSidebar()" aria-label="Open menu">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <main class="main-content">
        <section class="page active">
            <div class="page-header">
                <h1>Fulfillment</h1>
                <p>Purchase from your suppliers to restock items that are running low</p>
            </div>
            ...
            <div class="bill-section">
                ...
                <p class="settings-desc" id="restock-hint">Items at or below 5 units in stock are flagged here.</p>
                ...
                <div class="table-wrap">
                    <table class="data-table">
                        <thead>
                            <tr><th></th><th>Item</th><th>Current Stock</th><th>Status</th><th>Order Qty</th></tr>
                        </thead>
                        <tbody id="restock-table-body"></tbody>
                    </table>
                    <div id="restock-empty" class="placeholder-content" style="display:none;">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
                        <p>Nothing is low on stock right now. All tracked items are healthy.</p>
                    </div>
                </div>

                <button class="action-btn btn-save" style="margin-top:12px;" onclick="createPurchaseOrder()">
            ...
            <div class="bill-section">
                ...
                <div class="table-wrap">
                    <table class="data-table">
                        <thead>
                            <tr><th>PO No.</th><th>Date</th><th>Supplier</th><th>Items</th><th>Status</th><th></th></tr>
                        </thead>
                        <tbody id="po-table-body"></tbody>
                    </table>
                    <div id="po-empty" class="placeholder-content" style="display:none;">
                        ...
                        <p>No purchase orders yet. Create one from the restock list above.</p>
                    </div>
                </div>
            </div>
        </section>
    ...
    <footer class="app-footer">
        <div class="footer-content">
            ...
            <p class="footer-tagline">Simple billing, done right. v4.01.0</p>
        </div>
    </footer>
    ...
    <div id="po-modal" class="modal">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h3 id="po-modal-title">Purchase Order</h3>
            ...
            <div class="modal-footer">
                <button class="action-btn btn-clear" onclick="closePoModal()">Close</button>
            </div>
        </div>
    </div>
    ...
    <div id="confirm-modal" class="modal">
        ...
        <div class="modal-content" style="max-width:400px;">
            ...
            <div class="modal-body">
                <p id="confirm-modal-message" style="color:var(--text-secondary);line-height:1.6;">Are you sure?</p>
            </div>
            <div class="modal-footer">
                <button class="action-btn btn-danger" id="confirm-yes-btn">Confirm</button>
                <button class="action-btn btn-clear" onclick="closeConfirmModal()">Cancel</button>
            </div>
        </div>
    </div>
...
async function loadFulfillmentData() {
    ...
    try {
        const suppliers = await dbGet('suppliers');
        FSuppliers = suppliers || [];
    } catch (e) { FSuppliers = []; }

    try {
        const pos = await dbGet('purchaseOrders');
        PurchaseOrders = pos || [];
...
async function savePOStorage() {
    await dbSet('purchaseOrders', PurchaseOrders);
}

async function saveItemsStorage() {
    await dbSet('items', FItems);
}

async function saveStockLogEntry(itemId, itemName, type, qty, reference) {
    let log = [];
    try {
        const saved = await dbGet('stocklog');
        log = saved || [];
    } catch (e) { log = []; }
    log.unshift({
...
async function markPOReceived(id) {
    const po = PurchaseOrders.find(p => p.id === id);
    if (!po) return;
    if (!await showConfirm(`Mark ${po.poNumber} as received? This will add the ordered quantities to stock.`, 'Mark Received')) return;

    for (const line of po.items) {
        const item = FItems.find(i => String(i.id) === String(line.itemId));
        if (item) {
            item.stock = (item.stock || 0) + line.qty;
            await saveStockLogEntry(item.id, item.name, 'Purchase Received', line.qty, po.poNumber);
        }
    }
...
async function deletePO(id) {
    if (!await showConfirm('Delete this purchase order? This does not reverse any stock already received.', 'Delete PO')) return;
