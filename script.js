        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .pos-company-name { font-size: 14px; font-weight: 800; }
                .pos-gst, .pos-address, .pos-contact { font-size: 11px; font-weight: 700; line-height: 1.35; }
                .pos-divider { border: none; border-top: 2px dashed #000; margin: 5px 0; }
                .pos-meta { display: flex; justify-content: space-between; font-size: 11px; font-weight: 700; margin-bottom: 2px; }
                .pos-meta-label { font-weight: 800; }
                .pos-customer-label { font-weight: 800; font-size: 11px; }
                .pos-table { width: 100%; border-collapse: collapse; font-size: 11px; font-weight: 700; margin: 4px 0; }
                .pos-table th { border-top: 2px solid #000; border-bottom: 2px solid #000; padding: 3px 2px; text-align: left; font-weight: 800; }
                .pos-table td { padding: 2px 2px; vertical-align: top; font-weight: 700; }
                .pos-td-right { text-align: right; }
                .pos-td-center { text-align: center; }
                .pos-totals { border-top: 2px solid #000; padding-top: 3px; font-size: 11px; font-weight: 700; }
                .pos-total-row { display: flex; justify-content: space-between; margin-bottom: 1px; }
                .pos-total-row.grand { font-weight: 800; font-size: 13px; border-top: 2px solid #000; padding-top: 3px; margin-top: 2px; }
                .pos-saved { text-align: center; font-size: 11px; font-weight: 700; margin: 4px 0; font-style: italic; }
                .pos-payment { font-size: 11px; font-weight: 700; margin: 3px 0; }
                .pos-thankyou { text-align: center; font-size: 12px; font-weight: 800; margin: 4px 0; }
                .pos-terms { font-size: 10px; font-weight: 700; text-align: center; line-height: 1.35; margin-top: 3px; padding-top: 3px; border-top: 2px dashed #000; }
...
function exportAllData() {
    const bundle = {
        ...
        brands: JSON.parse(localStorage.getItem('billhive-brands') || '[]'),
        suppliers: JSON.parse(localStorage.getItem('billhive-suppliers') || '[]'),
        purchaseOrders: JSON.parse(localStorage.getItem('billhive-purchase-orders') || '[]')
    };
...
function importAllData(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (data.brands) localStorage.setItem('billhive-brands', JSON.stringify(data.brands));
            if (data.suppliers) localStorage.setItem('billhive-suppliers', JSON.stringify(data.suppliers));
            if (data.purchaseOrders) localStorage.setItem('billhive-purchase-orders', JSON.stringify(data.purchaseOrders));
...
function populateBrandsDatalist() {
    const dl = document.getElementById('brands-datalist');
    if (!dl) return;
    try {
        const saved = localStorage.getItem('billhive-brands');
